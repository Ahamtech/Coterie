    ...
    if (Q_LIKELY(!m_pSocket)) {
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 0, 2)
        bool isEmpty = url.query().isEmpty();
        QString query = url.query();
#else
        bool isEmpty = url.queryItems().isEmpty();
        QString query = "";
        ...
        if (url.scheme() == QStringLiteral("wss")) {
            if (!QSslSocket::supportsSsl()) {
                const QString message = ("SSL Sockets are not supported on this platform.");
                ...
                if (Q_LIKELY(m_pSocket)) {
                    ...
                    const QString message = ("Out of memory.");
                    setErrorString(message);
            ...
            if (url.scheme() == QStringLiteral("ws")) {
                ...
                const QString message = QString("Unsupported websockets scheme: %1").arg(url.scheme());
                setErrorString(message);
                Q_EMIT q->error(QAbstractSocket::UnsupportedSocketOperationError);
            }
    }
...
void QWebSocketPrivate::makeConnections(const QTcpSocket *pTcpSocket)
{
    ...
    if (Q_LIKELY(pTcpSocket)) {
        ...
        connect(pTcpSocket, SIGNAL(proxyAuthenticationRequired(QNetworkProxy,QAuthenticator*)), q, SIGNAL(proxyAuthenticationRequired(QNetworkProxy,QAuthenticator*)));
...
qint64 QWebSocketPrivate::doWriteFrames(const QByteArray &data, bool isBinary)
{
    ...
    for (int i = 0; i < numFrames; ++i) {
        ...
        if (Q_LIKELY(size > 0)) {
            ...
            if (Q_LIKELY(written > 0)) {
                ...
                m_pSocket->flush();
                setErrorString(QString("Error writing bytes to socket: %1.")
    ...
    if (Q_UNLIKELY(payloadWritten != data.size())) {
        setErrorString(QString("Bytes written %1 != %2.").arg(payloadWritten).arg(data.size()));
        Q_EMIT q->error(QAbstractSocket::NetworkError);
    }
    return payloadWritten;
}
...
#else
        foreach(QByteArray array, frames)
        {
            written += writeFrame(array);
        }
...
void QWebSocketPrivate::processHandshake(QTcpSocket *pSocket)
{
...
#else
    bool matches = statusLine.contains(regExp);
    regExp.indexIn(statusLine);
    ...
    if (Q_UNLIKELY(!ok)) {
        errorDescription = QString("Invalid statusline in response: %1.").arg(statusLine);
    } else {
        ...
        if (Q_LIKELY(httpStatusCode == 101)) {
...
#else
            const float version = httpProtocol.midRef(5).toString().toFloat(&conversionOk);
#endif
            ...
            if (ok) {
                ...
                if (!ok)
                    errorDescription =
                            QString("Accept-Key received from server %1 does not match the client key %2.")
                            .arg(acceptKey).arg(accept);
            } else {
                errorDescription =
                        QString("QWebSocketPrivate::processHandshake: Invalid statusline in response: %1.")
                        .arg(statusLine);
            }
            ...
            if (!version.isEmpty()) {
                ...
                if (!versions.contains(QString::number(QWebSocketProtocol::currentVersion()))) {
                    ...
                    errorDescription =
                            QString("Handshake: Server requests a version that we don't support: %1.")
                            .arg(versions.join(QStringLiteral(", ")));
                    ok = false;
                } else {
                    //we tried v13, but something different went wrong
                    errorDescription =
                            QString("QWebSocketPrivate::processHandshake: Unknown error condition " \
            ...
            errorDescription =
                    QString("QWebSocketPrivate::processHandshake: Unhandled http status code: %1 (%2).")
...
void QWebSocketPrivate::resume()
{
    ...
    if (Q_LIKELY(m_pSocket))
        m_pSocket->resume();
#else
    qWarning() << "Only in Qt5 supported";
#endif
}
