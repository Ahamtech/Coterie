...
#else
            connect(pSslServer,SIGNAL(newConnection()), this, SLOT(onNewConnection()));
...
void QWebSocketServerPrivate::setSslConfiguration(const QSslConfiguration &sslConfiguration)
{
    if (m_secureMode == SECURE_MODE)
        ...
        qWarning() << ("Cannot set SSL configuration for non-secure server.");
}
...
void QWebSocketServerPrivate::handshakeReceived()
{
    ...
    if (Q_LIKELY(pTcpSocket)) {
        ...
        if (m_pendingConnections.length() >= maxPendingConnections()) {
            pTcpSocket->close();
            qWarning() <<
                          ("Too many pending connections: new websocket connection not accepted.");
        ...
        if (request.isValid()) {
            ...
            if (response.isValid()) {
                ...
                setError(QWebSocketProtocol::CC_PROTOCOL_ERROR, ("Invalid response received."));
            }
        }
        if (!success) {
            qWarning() << ("Closing socket because of invalid or unsupported request.");
            pTcpSocket->close();
        }
    } else {
        qWarning() <<
                      ("Sender socket is NULL. This should not happen, otherwise it is a Qt bug!!!");
    }
}
