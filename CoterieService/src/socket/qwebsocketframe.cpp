...
bool QWebSocketFrame::checkValidity()
{
    ...
    if (Q_UNLIKELY(m_rsv1 || m_rsv2 || m_rsv3)) {
        setError(QWebSocketProtocol::CC_PROTOCOL_ERROR, QString("Rsv field is non-zero"));
    } else if (Q_UNLIKELY(QWebSocketProtocol::isOpCodeReserved(m_opCode))) {
        setError(QWebSocketProtocol::CC_PROTOCOL_ERROR, QString("Used reserved opcode"));
    } else if (isControlFrame()) {
        if (Q_UNLIKELY(m_length > 125)) {
            setError(QWebSocketProtocol::CC_PROTOCOL_ERROR,
                     QString("Controle frame is larger than 125 bytes"));
        } else if (Q_UNLIKELY(!m_isFinalFrame)) {
            setError(QWebSocketProtocol::CC_PROTOCOL_ERROR,
                     QString("Controle frames cannot be fragmented"));
        } else {
            m_isValid = true;
        }
    } else {
